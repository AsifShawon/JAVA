...
public class Admin_Panel implements ActionListener{
	...
	private JButton newVote,database,menu,EndCurrentVote,ParticipantDatabase;
	...
	// containers 
		...
		ParticipantDatabase = new JButton("Participant Database");
		ParticipantDatabase.addActionListener(this);
		ParticipantDatabase.setFont(new Font("Cambria", Font.BOLD, 18));
		ParticipantDatabase.setBounds(183, 148, 220, 51);
		c.add(ParticipantDatabase);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==ParticipantDatabase) {
			new PartDatabase();
			frame.dispose();
		}
		if(e.getSource()==EndCurrentVote) {
			int result = JOptionPane.showConfirmDialog(null, "Do you want to end "+RunnningVote.getVname()+"?","End vote",JOptionPane.OK_CANCEL_OPTION);
			if(result==JOptionPane.OK_OPTION) {
				RunnningVote.setPublished();
			}
		}
